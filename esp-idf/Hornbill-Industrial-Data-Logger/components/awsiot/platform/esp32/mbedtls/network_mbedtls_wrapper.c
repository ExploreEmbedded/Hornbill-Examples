...
extern const char *devicePrivateKey;
...
IoT_Error_t iot_tls_connect(Network *pNetwork, TLSConnectParams *params) {
	...
	ret = mbedtls_x509_crt_parse(&(tlsDataParams->cacert), (uint8_t*)rootCA, strlen(rootCA)+1);
	if(ret < 0) {
	...
	ret = mbedtls_x509_crt_parse(&(tlsDataParams->clicert), (const unsigned char *)deviceCert, strlen(deviceCert)+1);
	if(ret != 0) {
		IOT_ERROR(" failed\n  !  mbedtls_x509_crt_parse returned -0x%x while parsing device cert\n\n", -ret);
		return NETWORK_X509_DEVICE_CRT_PARSE_ERROR;
	}

	ret = mbedtls_pk_parse_key(&(tlsDataParams->pkey), (const unsigned char *)devicePrivateKey, strlen(devicePrivateKey)+1, (const unsigned char *)"", 0);
	if(ret != 0) {
